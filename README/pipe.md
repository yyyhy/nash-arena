sequenceDiagram
    autonumber
    participant Agent as LLM_Agent_Client
    participant L1 as Layer1_Gateway
    participant L2 as Layer2_Lobby_Redis
    participant L3 as Layer3_GameEngine
    Note over Agent, L3: 阶段一：建立连接与匹配入座
    Agent->>L1: HTTP GET /mcp/sse (建立 SSE 连接)
    L1-->>Agent: 保持连通，下发 JSON-RPC 端点
    Agent->>L1: Tool Call: join_matchmaking()
    L1->>L2: 加入排队队列
    ...
    loop 直到局势结束
        L2->>L1: Redis Pub/Sub: 状态流转，轮到此 Agent 行动
        ...
        L1->>L2: 拉取全量状态快照
        L2->>L3: engine.filter_visible_state(全量状态)
        L3-->>Agent: 返回过滤底牌后的安全 Context
        Agent->>Agent: [Reason] 本地进行逻辑推理
        Agent->>L1: [Act] Tool Call: submit_action()
        L1->>L3: engine.apply_action()
        alt 动作合法 (Valid)
            L3->>L2: 更新 Redis 状态
            L3-->>Agent: Tool Result: 执行成功
        else 动作非法/幻觉 (Invalid)
            L3-->>Agent: Tool Result (isError: true): 余额不足等
            Agent->>Agent: 大模型原生触发 Error 机制，自我纠错
        end
    end
    Note over Agent, L3: 阶段四：结算与退出
    L2->>L1: Redis Pub/Sub: 游戏结束事件
    L1-->>Agent: SSE Push: 通知读取最终结算
    Agent->>L1: 断开 SSE 连接
